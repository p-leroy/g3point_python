...
save_figure_folder = 'C:/Users/PaulLeroy/Nextcloud/Partages_recus/paul_leroy/Figures_ple/';
...
for i = 1 : n_iter
    ...
    dist = zeros(1, numel(Xgrid));
    iwol = zeros(1, numel(Xgrid));
    for k = 1 : numel(Xgrid)
...
for j = 1 : size(data, 1)
    sensorCenter = [data(j, 1), data(j, 2) + delta, data(j, 3)];
...
k = 0; 
for j = 1 : size(data, 1)
    sensorCenter = [data(j, 1), data(j, 2), data(j, 3) + delta ];
    %x-z plot
    k = k + 1;
    u(k) = data(j, 11);
    v(k) = data(j, 12);
...
figure
subplot(211);
hist(granulo.angle_Mview * 180 / pi);
xlabel('Azimut (°)');
axis tight;
subplot(212);
hist(granulo.angle_Xview * 180 / pi);
xlabel('Dip (°)');
axis tight;
xwidth = 10;
ywidth = 10;
